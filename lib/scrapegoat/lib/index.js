...
var eventTemplate = ejs.compile(xml.event);
...
Client.prototype.getEvents = function (events) {
    ...
    console.log('multi: ' + multiget);
    return request(this.config, 'REPORT', 1, multiget).then(xmlParser.parseEvents);
    //return request(this.config, 'REPORT', 1, multiget).then(
    //    function(res){
    //        console.log('res: ' + res);
    //    } ,function(err){
    //        console.log('er: ' + err);
 ...
 * @returns {Promise}
 */
Client.prototype.createEvent = function (task) {
    var template = eventTemplate({ task: task });
    //console.log("template: " + (JSON.stringify(template, null, 4)));
    return request(this.config, 'PUT', 1, template).then(function (res) {console.log('res: ' + res)});
};
