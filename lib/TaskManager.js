...
var update = exports.update = function update(tasks) {
    ...
    for (i = 0; i < tasks.length; i++) {
        ...
        console.log(`strt ${t.id} ${t.description} project:${t.project} tags:${t.tags} ${t.start}`);
        if (!tagExists(t, inProgress) && t.start != null) { // if task is started, add "in progress" tag
            addTag(t, inProgress);
        } else if (tagExists(t, inProgress) && t.start == null && t.status != 'completed') { // if "in progress" tag, start the task
            executeCommand(`${t.id} start`);
        }
...
var editTask = exports.editTask = function editTask(id, description, due, priority, project, tags, status, start) {
    ...
    if ((tags.indexOf(inProgress) == -1) && start == 'dragged') { //if task is no longer in progress - stop it
