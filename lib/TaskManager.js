...
var deleteTask = exports.deleteTask = function deleteTask(id){
    ...
    var del = exec(`task delete ${id}`, function (err, stdout, stderr) {
        if (err) {
            console.log(`Command "task delete ${id}" failed with message:  ${err.message}`)
        }
        else
        {
            console.log(`Command "task delete ${id}" executed successfully`)
        }
    });
    del.stdin.write(`yes\n`);
}
